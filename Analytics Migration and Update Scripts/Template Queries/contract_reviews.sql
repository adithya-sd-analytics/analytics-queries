...
v5_starts as
(select approval_id, created, action from `{{project_id}}.{{prod_dataset_name}}.{{public}}approvals_v5_approvalv5actions`
where is_deleted = false
and action = 'start'
),

v5_reassign as 
(select required_id, created, on_hold, created_by_id from `{{project_id}}.{{prod_dataset_name}}.cron_audit_table`
...
from
  ...
  from 
    ...
    left join `{{project_id}}.{{prod_dataset_name}}.{{public}}sd_organizations_organizationuser` as c on a.created_by_org_user_id = c.id
...
left join `{{project_id}}.{{prod_dataset_name}}.{{public}}sd_auth_role` as d on a.role_id = d.id
