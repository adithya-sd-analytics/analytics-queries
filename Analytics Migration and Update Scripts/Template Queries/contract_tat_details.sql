...
on_hold_tab as 
(select * from `{{project_id}}.{{prod_dataset_name}}.on_hold_non_work_days`
),
...
Review_completed_on, coalesce(Review_completed_on, current_timestamp) as end_timestamp_current, timestamp_diff(coalesce(review_completed_on, current_timestamp), Review_requested_on, SECOND) as time_spent  from `{{project_id}}.{{Analytics_base_dataset_name}}.Contracts_review_table`),
