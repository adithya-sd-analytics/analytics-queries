...
select contract_id, audit_type, created as on_hold_start, next_ts as on_hold_ends, coalesce(next_ts, current_timestamp) as on_hold_ends_current from 
  ...
  from
    ...
    from `spotdraft-prod.prod_india_db.cron_audit_table`
    where 
...
sent_for_sign as 
(select * from 
  (select *, row_number() over(partition by required_id order by created desc) as rn from `spotdraft-prod.prod_india_db.cron_audit_table`  
where audit_type in ('sent-for-signature-in-order', 'sent-for-signature'))
where rn = 1 ),

signs_completed_on as
(select * from 
(select *, row_number() over(partition by sign_recipient_id order by created desc) as rn from `spotdraft-prod.prod_india_db.public_contracts_signatorysignaturedata`
where is_success
...
field_created as 
(select contract_id, max(modified) as last_updated from `spotdraft-prod.prod_india_db.public_contracts_v3_contractsignaturesetup`
...
from `spotdraft-prod.prod_india_db.public_contracts_v3_contractv3signatory` as a 
...
join `spotdraft-prod.prod_india_db.public_contracts_v3_signrecipient` as d on a.sign_recipient_id = d.id
left join field_created as f on a.contract_id = f.contract_id
left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as org on org.id = a.org_user_id
left join `spotdraft-prod.prod_india_db.public_sd_organizations_companyprofile` as com on org.organization_id = com.owner_id
...
join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as d 
...
raw_approval_timeline as (
 ...
 from (
    ...
    from `spotdraft-prod.prod_india_db.public_contracts_v3_contractapprovalv2`
 ...
 union all (
    ...
    left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as b on a.created_by_id = b.user_id
...
from `spotdraft-prod.prod_india_db.public_contracts_v3_contractrequiredapprovalv2` as a 
left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as b on a.org_user_id = b.id
left join `spotdraft-prod.prod_india_db.public_sd_auth_role` as c on a.role_id = c.id
...
sign_reset as 
(
(select created,  null as recipient_id, 'Reset move to redlining' as action,contract_id, created_by_workspace_id  from `spotdraft-prod.prod_india_db.state_changes_table`
...
from `spotdraft-prod.prod_india_db.public_contracts_v3_recipientaction`
...
from `spotdraft-prod.prod_india_db.public_contracts_v3_contractversion`
...
(select * from (select concat(first_name,' ' ,last_name) as user_name, user_id, id, row_number() over(partition by user_id order by created desc) as rn
 from `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser`)where rn = 1) as b on a.sent_by_id = b.user_id
left join `spotdraft-prod.prod_india_db.public_contracts_v3_signrecipient` as c on a.required_id = c.id
left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as d on c.org_user_id = d.id
left join `spotdraft-prod.prod_india_db.public_sd_organizations_companyprofile` as e on d.organization_id = e.owner_id),
...
from app_data_with_time as a
  left join `spotdraft-prod.prod_india_db.public_contracts_v3_contractrequiredapprovalv2` as d --joins for approval name
  on a.required_approval_id = d.id
  left join `spotdraft-prod.prod_india_db.public_contracts_v3_contractrequiredfullapproval` as e  --joins for approval name
  on d.required_approval_id = e.id
  left join v4_pending_with as j on a.required_approval_id = j.id
  left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as h --table for user names
