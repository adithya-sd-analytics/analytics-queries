...
entity as
(select contract_id, name as entity from `spotdraft-prod.prod_india_db.public_contracts_v3_contractrole` as a 
join `spotdraft-prod.prod_india_db.public_sd_organizations_v2_organizationentity` as b on a.organization_entity_id = b.id
where role = 'CONTRACTOR'),

redline_entry as 
(select distinct contract_id from `spotdraft-prod.prod_india_db.cron_audit_table`
...
`spotdraft-prod.prod_india_db.public_contracts_v3_contractuser` as a 
...
left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as c 
on a.org_user_id = c.id
left join `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser` as d 
...
exec_date as (
  ...
  from (
    ...
    from `spotdraft-prod.prod_india_db.public_contracts_v3_contractkeypointer`
    where key_pointer_id in (
      select 
        id 
      from `spotdraft-prod.prod_india_db.public_historic_contracts_keypointer`
...
expiry_date as (
  ...
  from (
    ...
    from `spotdraft-prod.prod_india_db.public_contracts_v3_contractkeypointer`
    where key_pointer_id in (
      select id from `spotdraft-prod.prod_india_db.public_historic_contracts_keypointer`
...
term_length as (
  ...
  and id not in (select id from `spotdraft-prod.prod_india_db.public_contracts_v3_contractkeypointer` where cast(json_extract_scalar(value, '$.days')as float64) > 36500)
  ) as a 
  where rn = 1
),
...
from `spotdraft-prod.prod_india_db.public_contracts_v3_contractrole` as a 
join `spotdraft-prod.prod_india_db.public_sd_organizations_companyprofile` as b on a.workspace_id = b.id
...
-- tl.term_length_days, 
  ...
  left join `spotdraft-prod.prod_india_db.public_contracts_v3_workspacecontracttype` as ct on a.contract_type_id = ct.contract_type_id and a.created_by_workspace_id = ct.workspace_id
  left join `spotdraft-prod.prod_india_db.public_contracts_templateworkspaceaccess` as ty on a.contract_template_id = ty.contract_template_id and a.created_by_workspace_id = ty.workspace_id
  left join (
    select * 
    from (
      select 
  ...
  left join (
    ...
    from (
      ...
      from `spotdraft-prod.prod_india_db.public_spot_insights_contractevent`
    ) as a 
    where rn = 1
  ) as ds 
...
from `spotdraft-prod.prod_india_db.public_public_externalintegrationcontractdetail`
...
(select a.entity_id contract_id, b.title as frozen_workflow_title, b.id as frozen_workflow_id,
 ...
 from `spotdraft-prod.prod_india_db.public_workflow_v1_frozenworkflowtoconsumerentitymapping` as a
join `spotdraft-prod.prod_india_db.public_workflow_v1_frozenworkflow` as b on a.frozen_workflow_id = b.id
join `spotdraft-prod.prod_india_db.public_workflow_v1_workflow` as c on b.workflow_id = c.id
...
left join `prod_india_db.Teams` as e on c.bussiness_org_id = e.id
left join `spotdraft-prod.prod_india_db.public_contracts_v3_bulkuploaddetail` as ae on ae.contract_id = a.id
