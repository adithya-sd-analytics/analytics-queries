...
SELECT 
    ...
    a.owner_id, 
    a.name 
FROM `spotdraft-prod.prod_india_db.public_sd_organizations_companyprofile` AS a
WHERE EXISTS (
    SELECT 1
    FROM `spotdraft-prod.prod_india_db.public_sd_organizations_enhancedflag_company_profiles` AS b
    INNER JOIN `spotdraft-prod.prod_india_db.public_sd_organizations_enhancedflag` AS c 
...
max( json_extract_scalar(value,'$')) as time_zone,
  ...
  from spotdraft-prod.prod_india_db.public_settings_manager_settingvalue
...
from `spotdraft-prod.prod_india_db.public_sd_organizations_v2_organizationentity`  as a
join relevant_workspaces as b on a.organization_id = b.owner_id
left join `spotdraft-prod.prod_india_db.public_geo_address` as e on a.primary_address_id = e.id
left join `spotdraft-prod.prod_india_db.public_geo_country` as f on e.country_id = f.id
left join `spotdraft-prod.prod_india_db.public_geo_country` as g on a.jurisdiction_id = g.id
...
left join `spotdraft-prod.prod_india_db.default_analytics_setting` as da on da.config_key = 'timezone'
left join `spotdraft-prod.prod_india_db.default_analytics_setting` as db on db.config_key = 'non_working_days'
...
from
    ...
    left join `spotdraft-prod.prod_india_db.timezone_table` as c on b.timezone = c.iana_timezone
