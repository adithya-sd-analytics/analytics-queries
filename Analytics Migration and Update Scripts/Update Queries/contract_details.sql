...
concat('https://app.spotdraft.com/contracts/v2/', ab.id) as contract_link
 from  `spotdraft-prod.prod_india_db.public_contracts_v3_contractv3` as ab
left join 
(select * from
...
left join `spotdraft-prod.prod_india_db.public_contracts_v3_workspacecontracttype` as ct 
on ab.contract_type_id = ct.contract_type_id and ct.workspace_id = ab.created_by_workspace_id
left join `spotdraft-prod.prod_india_db.public_contracts_templateworkspaceaccess` as tem  
...
(select * from
  (select user_id, first_name, last_name, row_number() over(partition by user_id order by created desc) as rn from `spotdraft-prod.prod_india_db.public_sd_organizations_organizationuser`)
where rn = 1
),
...
left join `spotdraft-prod.prod_india_db.Teams` as g on a.bussiness_org_id = g.id
