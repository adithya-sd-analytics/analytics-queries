...
join `spotdraft-prod.prod_india_db.public_contracts_v3_workspacecontracttype` as c 
...
left join `spotdraft-prod.prod_india_db.public_contracts_contracttemplate` as cty  
  on cty.id = b.contract_template_id 
left join contract_stages as cs on cs.contract_id = a.contract_id
left join `spotdraft-prod.prod_india_db.public_contracts_signatorysignaturedata` as ssd on a.id = ssd.contract_signatory_id)
